...
def process_detections(pred, webcam, path, im0s, img, deepsort, save_txt, txt_path, frame_idx, view_img, save_img, dataset, vid_path, save_path, vid_cap, vid_writer, opt):
...
def detect(opt, save_img=False):
    ...
    model = tv_models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        pred = model(img)[0]
        print(f"pred boxes: {pred['boxes']}, pred labels: {pred['labels']}")
        ...
        # Process detections
        # process_detections(pred, webcam, path, im0s, img, deepsort, save_txt, txt_path, frame_idx, view_img, save_img, dataset, vid_path, save_path, vid_cap, vid_writer, opt)

        # Print time (inference + NMS)
        print('Done. (%.3fs), %.3f' % (t2 - t1, (1.0/(t2 - t1))))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_deepsort", type=str, default="deep_sort/deep_sort.yaml")
